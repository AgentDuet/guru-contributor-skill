...
def cmd_get(ba_uid: str) -> int:
    entry = _load().get(ba_uid)
...
def cmd_list() -> int:
    ...
    if not data:
        ...
        return 0
    for ba_uid, entry in data.items():
        print(f"{ba_uid}  expires_at={entry.get('expires_at')}  display_name={entry.get('display_name')}  "
...
def main(argv: list[str]) -> int:
    ...
    if cmd in ("get", "show", "set"):
        if len(argv) != 3:
            print(f"creds.py {cmd}: needs <ba_uid>", file=sys.stderr)
            return 2
